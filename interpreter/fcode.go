...
// of the stack) produces a legal allocation of values to frame slots: each value's ifSrc1Frame
...
const (
	...
	vfMaterialized             // this value has been materialized (i.e. its producing instruction is part of the function)
...
// use as frame sources for instructions, and are typically generated by local.get
// instructions. A frame address or constant value may need to be materialized if its
// value must be on the frame e.g. because it is passed to or returned from a block or
// call, or refers to a frame slot that is reassigned before the value is used (e.g. a
// local.get that is not used prior to a local.set that targets the same local.
type fvalue struct {
...
func (imp *fimporter) emitLoad(instr *code.Instruction) {
	...
	// If the address is a constant, add it to the load offset and use the constant form.
	if fi.src2 == 0 {
		fi.opcode |= 0x0200
	}
...
func (imp *fimporter) emitStore(instr *code.Instruction) {
	...
	value, address := imp.popAddressable(), imp.popAddressable()
	...
	// If the address is a constant, add it to the store offset and use the constant form.
	if fi.src2 == 0 {
		fi.opcode |= 0x0200
	}
